  ...
  $.fn.gridEditor = function (options) {
    ...
    self.each(function (baseIndex, baseElem) {
      ...
      function setup() {
        ...
        htmlTextArea = $('<textarea class="ge-html-output"/>').insertBefore(
          canvas
        );
        ...
        // Add row
        addRowGroup = $('<div class="ge-addRowGroup btn-group" />').appendTo(
          wrapper
        );
        ...
        // Buttons on right
        var layoutDropdown = $(
          '<div class="dropdown pull-right ge-layout-mode">' +
        ...
        var htmlButton = $(
          ...
          .appendTo(btnGroup);
        var previewButton = $(
          '<button title="Preview" type="button" class="btn btn-sm btn-primary gm-preview"><i class="fa fa-eye"></i></button>'
        )
      ...
      function initRTE(e) {
        if ($(this).hasClass("ge-rte-active")) {
          return;
        }
      ...
      function createRowControls() {
        canvas.find(".row").each(function () {
          ...
          var details = createDetails(row, settings.row_classes).appendTo(
            drawer
          );
        });
      }
        ...
        canvas.find(".column").each(function () {
          ...
          createTool(
            ...
            function (e) {
              ...
              var curColSizeIndex = colSizes.indexOf(
                getColSize(col, curColClass)
              );
              var newSize =
                colSizes[clamp(curColSizeIndex - 1, 0, colSizes.length - 1)];
              if (e.shiftKey) {
                newSize = colSizes[0];
              }
          ...
          var details = createDetails(col, settings.col_classes).appendTo(
            drawer
          );
        });
      }
        ...
        cssClasses.forEach(function (rowClass) {
          var btn = $('<a class="btn btn-sm btn-default" />')
            ...
            .attr(
              ...
              rowClass.title
                ? rowClass.title
                : 'Toggle "' + rowClass.label + '" styling'
            )
            .toggleClass(
              "active btn-primary",
              container.hasClass(rowClass.cssClass)
            )
            .on("click", function () {
      ...
      function createColumn(size) {
        return $("<div/>")
          ...
          .append(
            createDefaultContentWrapper().html(
      ...
      function doWrap(contents) {
        if (contents.length) {
          var container = createDefaultContentWrapper().insertAfter(
            contents.last()
          );
      ...
      contentAreas.each(function () {
        ...
        if (!contentArea.hasClass("active")) {
          ...
          var configuration = $.extend(
            {},
            settings.ckeditor && settings.ckeditor.config
              ? settings.ckeditor.config
              : {},
            {
